...
// Use client helper to avoid bundling prisma into the browser
import { createResumeClient } from "@/actions/resumes.client";
...
export default function CRMResumesClient({ user, resumes: initialResumes }: Props) {
  ...
  const handleCreate = useCallback(async () => {
    setCreating(true);
    try {
      const resume = await createResumeClient({ title: "Untitled Resume", content: "{}" });
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <button
            ...
            disabled={creating}
            className="px-5 py-2.5 bg-gradient-to-r from-purple-500 to-purple-700 text-white text-sm font-medium rounded-xl hover:from-purple-600 hover:to-purple-800 disabled:opacity-50 transition-all"
          >
