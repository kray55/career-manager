    ...
    <>
      ...
      {isOpen && (
        <div className="fixed bottom-20 right-4 z-50 w-[380px] h-[520px] bg-slate-900 border border-white/10 rounded-2xl shadow-2xl flex flex-col overflow-hidden">
          ...
          <div className="flex items-center gap-2 px-4 py-2 border-b border-white/10 bg-slate-950/30">
            <span className="text-[10px] uppercase tracking-wide text-slate-500 mr-auto">Room tools</span>
            <button onClick={() => setShowAvatarPicker(!showAvatarPicker)} className="px-2 py-1 text-[10px] text-slate-200 bg-slate-800 border border-white/10 rounded hover:bg-slate-700" title="Choose your user avatar">Avatar {avatarEmoji}</button>
            <button onClick={() => setShowInvite(!showInvite)} disabled={!activeRoomId} className="px-2 py-1 text-[10px] text-primary-200 bg-primary-500/10 border border-primary-500/30 rounded disabled:opacity-40" title={activeRoomId ? "Create a guest invitation" : "Select a room to invite a guest"}>Invite Guest</button>
          </div>
